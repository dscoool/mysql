USE db;
select * from enroll_info;
SELECT * FROM enroll_info ORDER BY `총공사금액(억원)`;
SELECT * FROM enroll_info ORDER BY `총공사금액(억원)` DESC;

-- parking 테이블에서 'amount'열을 지정하여 내림차순(DESC)으로 정렬합니다!!  
SELECT * FROM enroll_info ORDER BY `공사종료일` DESC;
SELECT * FROM enroll_info ORDER BY `공사시작일` ASC;
...
SELECT DISTINCT 공사종료일 from enroll_info;
SELECT DISTINCT 공사시작일 from enroll_info;
