...
INSERT INTO cctv.wifi ( `num`, `add`, `add2`,
...
DELETE FROM `cctv`.`wifi` WHERE (`num`='256');

UPDATE `cctv`.`wifi` SET `add2` = '1F 민원실' WHERE (`num` = '255');

SELECT * FROM cctv.wifi WHERE (`add` = '보건소');

SELECT * FROM cctv.wifi WHERE (`district` = '강북구');

-- SQL은 대소문자 구분 안함!! (보통 대문자를 씁니다)
select `add`, `add2`, `SSID` from cctv.wifi where (`add` = '보건소');
